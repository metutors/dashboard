...
export async function buildDashboardWorkbook(
  ...
  const summaryRows: Array<{ metric: string; value: string | number }> = [
    ...
    { metric: "People Filter", value: data.filters.peopleLabel ?? "All" },
    { metric: "Module Filter", value: data.filters.moduleLabel ?? "All" },
    { metric: "Sub Module Filter", value: data.filters.subModuleLabel ?? "All" },
